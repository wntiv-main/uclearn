import { DEBUG, DEBUG_HYDRATION } from './debug-options'
import { Hirschberg } from './hirschberg'
import { getRequire, getYUIInstance } from './lib-hook'
...
import { assertNever, compareClasses, isElement } from './util';
import { DOMInspector } from './dominspect';

let videojs: typeof VideoJS;
let ModalFactory: UCModalFactory;
let ModalEvents: UCModalEvents;
let Toast: UCToast;
	...
	if(isElement(el)) {
		...
		return {
			...
			...(el.classList.contains('video-js') ? {
					nodeType: HTMLChildType.VIDEOJS,
					sources: 'player' in el ? (el as VideoJSElement).player.currentSources() : videojs.getComponent("Player").getTagSettings(el).sources,
...
async function hydrate(dom: Element, updated: Element, nameHint: string) {
		...
		scriptLoadCompletions.push(new Promise((res, rej) => {
			...
			if(DEBUG) {
				...
				el.addEventListener("error", e => {
					...
					Toast.add('Script failed to run during hydration!', { type: 'danger' });
				});
			}
	...
	for(const script of scripts) {
		...
		const el = document.createElement("script");
		if(DEBUG) el.addEventListener("error", e => {
			console.warn("Script failed to run:", e, el);
			Toast.add('Script failed to run during hydration!', { type: 'danger' });
		});
		el.text = script.text;
...
async function hydrateFromResponse(resp: Response, hydrationHints: string[] = []) {
	...
	for(const selector of hydrationHints) {
		...
		if(current.length !== 1 || other.length !== 1) {
			if(DEBUG) {
				...
				Toast.add(`Hydration hint '${selector}' failed, selecting (${current.length}, ${other.length}) nodes`, { type: 'warning' });
			}
	...
	Toast.add(`Hydration in ${((performance.now() - startTime) / 1000).toFixed(2)}s`, { type: 'success' });
}
...
export async function initHydration() {
	...
	[videojs, ModalFactory, ModalEvents, Toast] = await new Promise(res => require(
		["media_videojs/video-lazy", "core/modal_factory", "core/modal_events", "core/toast"],
		(...args: [typeof VideoJS, UCModalFactory, UCModalEvents, UCToast]) => res(args)));
